...
std::pair<uint8_t, uint8_t> modbusrtu::plausibleTwo(const std::string &fileName,
  ...
  std::vector<uint8_t> dataBuffer{std::istreambuf_iterator<char>(file),
...
std::pair<uint8_t, uint8_t> modbusrtu::smartTwo(const std::string &fileName,
                                                int stride) {
  std::ifstream file(fileName, std::ios::binary);
  if (!file)
    return {248, 248};

  const std::vector<uint8_t> dataBuffer{std::istreambuf_iterator<char>(file),
                                        std::istreambuf_iterator<char>()};
        ...
        if (iter + 1 >= dataBuffer.size()) {
          ...
          case 0x10: {
            if (dataBuffer.size() > iter + 7) {
              uint8_t msgCnt[] = {candidate,
                                  function,
                                  dataBuffer.at(iter + 2),
                                  dataBuffer.at(iter + 3),
                                  dataBuffer.at(iter + 4),
                                  dataBuffer.at(iter + 5)};
              uint8_t crcLow = dataBuffer.at(iter + 6);
              uint8_t crcHigh = dataBuffer.at(iter + 7);
              uint16_t crc = (crcLow << 8) | crcHigh;
              if (uint16_t guessCrc = crcCalculation(msgCnt, 6);
                  guessCrc == crc) {
                score += 100;
              } else {
                score -= 100;
...
modbusrtu::confidenceMatrix(const std::pair<uint8_t, uint8_t> &guessSetone,
                            const std::pair<uint8_t, uint8_t> &guessSettwo) {
  return std::array{ba2{(guessSetone.first == guessSettwo.first),
                        (guessSetone.first == guessSettwo.second)},
                    ba2{(guessSetone.second == guessSettwo.first),
                        (guessSetone.second == guessSettwo.second)}};
}

std::vector<modbusrtu::modbusPdu>
modbusrtu::lexCapture(const std::string &fileName) {
  std::vector<modbusrtu::modbusPdu> returnable = {};
  auto worker1 = std::async(std::launch::async, &plausibleTwo, fileName, 5);
  auto worker2 = std::async(std::launch::async, &smartTwo, fileName, 5);
  ...
  std::ifstream file(fileName, std::ios::binary);
  if (histoRes.first == 248 || histoRes.second == 248 ||
      smartRes.first == 248 || smartRes.second == 248 || !file) {
    const devAddr eAddr = {ModbusAddr(0), "Error: No File Found"};
    const functionInfo eFunc = {"Error: No File Found", 0x00,
                                modbusFunc::Unknown, 0, 0};
    modbusPdu errorPDU = {eAddr, eFunc, {}, 0x0000, true};
    return {errorPDU};
  }
  ...
  for (size_t iter = start; iter < bytes.size(); iter++) {
    if (checkValues.contains(bytes.at(iter))) {
      if (iter + 1 < bytes.size()) {
        ...
        devAddr dAddr = {addr, addr.getName()};
        functionInfo function = getFunctionType(fCode);
        modbusPdu wPdu{dAddr, function, {}, 0x0000, true};
        ...
        if (iter + function.RequestSize <= bytes.size()) {
          ...
          for (size_t inner = iter; inner < iter + function.RequestSize - 2;
               inner++) {
          ...
          if (crc == crcGuess) {
            ...
            for (size_t responseSize = function.ResponseSize;
                 iter + responseSize <= 256 &&
                 iter + responseSize <= bytes.size();
                 responseSize++) {
              std::vector<uint8_t> body2;
              for (size_t inner = iter; inner < iter + responseSize - 2;
                   inner++) {
...
std::vector<modbusrtu::stringifyPdu>
modbusrtu::stringify(std::vector<modbusPdu> input) {
  std::vector<modbusrtu::stringifyPdu> returnable;
  for (auto [Addr, Func, Data, CRC, Request] : input) {
    stringifyPdu strung = {Addr.modbusAddr.getName(), Func.FunctionName, "",
                           std::format("CRC: 0x{:04X}", CRC), "request"};
