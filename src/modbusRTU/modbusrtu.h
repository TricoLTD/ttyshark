  ...
  explicit ModbusAddr(const uint8_t Addr) : Address(Addr) {
    if (Addr > 247) {
      throw std::out_of_range(
          std::format("Address was not valid 0x{:02X}", Addr));
    }
  };
...
public:
  ...
  static functionInfo getFunctionType(const uint8_t code) {
    ...
    case 0x10:
      value.FunctionName =
          std::format("Write Multiple Registers 0x{:02X}", code);
      value.FunctionID = code;
    ...
    case 0x17:
      value.FunctionName =
          std::format("Read/Write Multiple Registers 0x{:02X}", code);
      value.FunctionID = code;
      value.FunctionType = modbusFunc::ReadCoils;
      value.RequestSize = 0;
      value.ResponseSize = 0;
   ...
   * @return
   */
  static std::pair<uint8_t, uint8_t> plausibleTwo(const std::string &fileName,
                                                  std::size_t stride = 5);
   ...
   * @return
   */
  static std::pair<uint8_t, uint8_t> smartTwo(const std::string &fileName,
                                              int stride = 5);
   ...
   * @return
   */
  static std::array<ba2, 2>
  confidenceMatrix(const std::pair<uint8_t, uint8_t> &guessSetone,
  ...
  static void coloredPrint(const stringifyPdu &pdu);
};
