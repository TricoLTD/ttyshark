...
void serialcap::start(unsigned int await, unsigned int chunkness) {
  ...
  worker_ =
      std::async(std::launch::async, &serialcap::run, this, await, chunkness);
}
...
auto serialcap::run(unsigned int await, unsigned int chunkness)
  ...
  if (flock(fd_, LOCK_EX | LOCK_NB) == -1) {
    return std::unexpected(std::runtime_error(
        "Serial port with file descriptor " + std::to_string(fd_) +
...
// OFLAGS
  ...
  if (tcsetattr(fd_, TCSANOW, &tty) != 0) {
    ...
    return std::unexpected(std::runtime_error(
        "Error " + std::to_string(errno) + "from tcsetattr" + strerror(errno)));
  }
...
auto serialcap::open_port() -> serialcap::status {
  ...
  if (fd_ == -1) {
    return std::unexpected(
        "Device inaccessible! is it correct and do you have permissions?");
  }

  return 1;
}

void serialcap::close_port() { fd_ = ::close(fd_); }
